...
console.log('API Key longitud:', API_KEY ? API_KEY.length : 0);
...
async function fetchLeagueStandings(leagueId, season, isFirst) {
  const url = `${BASE_URL}/standings?league=${leagueId}&season=${season}`;
  try {
    const res = await fetch(url, { headers });
    const data = await res.json();

    // Solo logueamos la primera liga para ver la respuesta completa
    if (isFirst) {
      console.log('── DEBUG PRIMERA LIGA ──');
...
async function main() {
  ...
  for (const league of LEAGUES) {
    ...
    const teams = await fetchLeagueStandings(league.id, league.season, isFirst);
    isFirst = false;
